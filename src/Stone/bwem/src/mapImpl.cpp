...
static bool adjoins8SomeLakeOrNeutral(WalkPosition p, const MapImpl * pMap)
{
								...
								WalkPosition(-1, +1), WalkPosition(0, +1), WalkPosition(+1, +1)})
	{
		...
		if (pMap->Valid(next))
		{
			if (pMap->GetTile(TilePosition(next), no_check).GetNeutral()) return true;
			if (pMap->GetMiniTile(next, no_check).Lake()) return true;
		}
	}
...
///	Timer timer;
	...
	for (TilePosition pos : bw->getStartLocations())
		m_StartingLocations.push_back(pos);
...
// Computes walkability, buildability and higherGround information, using BWAPI corresponding functions
	...
	for (int x = 0 ; x < WalkSize().x ; ++x)
		if (!bw->isWalkable(x, y))						// For each unwalkable minitile, we also mark its 8 neighbours as not walkable.
			...
			for (int dx = -1 ; dx <= +1 ; ++dx)
			{
				...
				if (Valid(w))
					GetMiniTile_(w, no_check).SetWalkable(false);
			}
	...
	for (int x = 0 ; x < Size().x ; ++x)
	{
		...
		if (bw->isBuildable(t))
		{
			...
			for (int dx = 0 ; dx < 4 ; ++dx)
				GetMiniTile_(WalkPosition(t) + WalkPosition(dx, dy), no_check).SetWalkable(true);
		}

	// Mark higher ground tiles (tiles are in lower ground by default)
		if (bw->getRegionAt(Position(t))->isHigherGround())
			GetTile_(t).SetHigherGround();
	}
}
...
void MapImpl::DecideSeasOrLakes()
{
	...
	for (int x = 0 ; x < WalkSize().x ; ++x)
	{
		...
		MiniTile & Origin = GetMiniTile_(origin, no_check);
		if (Origin.SeaOrLake())
		{
			...
			while (!ToSearch.empty())
			{
				...
				for (WalkPosition delta : {WalkPosition(0, -1), WalkPosition(-1, 0), WalkPosition(+1, 0), WalkPosition(0, +1)})
				{
					...
					if (Valid(next))
					{
						MiniTile & Next = GetMiniTile_(next, no_check);
						if (Next.SeaOrLake())
						{
							ToSearch.push_back(next);
							if (SeaExtent.size() <= lake_max_size) SeaExtent.push_back(&Next);
							Next.SetSea();
						}
					}
				}
			...
			if ((SeaExtent.size() <= lake_max_size) &&
				(bottomRight.x - topLeft.x <= lake_max_width) &&
				(bottomRight.y - topLeft.y <= lake_max_width) &&
...
{
	MiniTile & Origin = GetMiniTile_(p, no_check);
...
void MapImpl::ComputeAltitude()
{
	...
	const int range = max(WalkSize().x, WalkSize().y) / 2 + 3;		// should suffice for blank maps
	...
	for (const auto & delta_altitude : DeltasByAscendingAltitude)
	{
		...
		for (int i = 0 ; i < (int)ActiveSeaSideList.size() ; ++i)
		{
			...
			if (altitude - Current.lastAltitudeGenerated >= 2 * altitude_scale)		// optimization : once a seaside miniTile verifies this condition, 
				fast_erase(ActiveSeaSideList, i--);									// we can throw it away as it will not generate altitudes anymore
			else
									...
									WalkPosition(d.y, d.x), WalkPosition(-d.y, d.x), WalkPosition(d.y, -d.x), WalkPosition(-d.y, -d.x)})
				{
					...
					if (Valid(w))
					{
						auto & miniTile = GetMiniTile_(w, no_check);
...
void MapImpl::ProcessBlockingNeutrals()
{
	...
	for (Neutral * pCandidate : Candidates)
		if (!pCandidate->NextStacked())		// in the case where several neutrals are stacked, we only consider the top
		{
			...
			really_remove_if(Border, [this](WalkPosition w)	{
				return !Valid(w) || !GetMiniTile(w, no_check).Walkable() ||
					GetTile(TilePosition(w), no_check).GetNeutral(); });
				...
				while (!ToVisit.empty())
				{
					...
					for (WalkPosition delta : {WalkPosition(0, -1), WalkPosition(-1, 0), WalkPosition(+1, 0), WalkPosition(0, +1)})
					{
						...
						if (Valid(next) && !contains(Visited, next))
							if (GetMiniTile(next, no_check).Walkable())
								if (!GetTile(TilePosition(next), no_check).GetNeutral())
			...
			if (TrueDoors.size() >= 2)
			{
				// Marks pCandidate (and any static building stacked with him) as blocking
...
// The miniTiles are considered consecutively in descending order of their Altitude().
// Each of them either:
...
vector<pair<WalkPosition, MiniTile *>> MapImpl::SortMiniTiles()
{
	...
	for (int x = 0 ; x < WalkSize().x ; ++x)
	{
		...
		MiniTile & miniTile = GetMiniTile_(w, no_check);
...
static pair<Area::id, Area::id> findNeighboringAreas(WalkPosition p, const MapImpl * pMap)
{
	...
	for (WalkPosition delta : {WalkPosition(0, -1), WalkPosition(-1, 0), WalkPosition(+1, 0), WalkPosition(0, +1)})
		...
		{
			Area::id areaId = pMap->GetMiniTile(p + delta, no_check).AreaId();
			if (areaId > 0)
...
void MapImpl::SetAreaIdInTile(TilePosition t)
{
	...
	for (int dy = 0 ; dy < 4 ; ++dy)
	for (int dx = 0 ; dx < 4 ; ++dx)
		if (Area::id id = GetMiniTile(WalkPosition(t) + WalkPosition(dx, dy), no_check).AreaId())
...
void MapImpl::SetAltitudeInTile(TilePosition t)
{
	...
	for (int dx = 0 ; dx < 4 ; ++dx)
	{
		altitude_t altitude = GetMiniTile(WalkPosition(t) + WalkPosition(dx, dy), no_check).Altitude();
	...
	for (auto location : StartingLocations())
	{
		...
		for (Area & area : GetGraph().Areas()) if (!found)
			for (Base & base : area.Bases()) if (!found)
				if (queenWiseDist(base.Location(), location) <= 3)
				{
