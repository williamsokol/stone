...
public:
	...
	// Returns 0 if cpA == cpB or if cpA is not accessible from cpB
	...
	// The path always starts with cpA and ends with cpB, unless !cpA->AccessibleFrom(cpB): an empty list is then returned.
...
//	The functions below are reserved for the Map initialization process
