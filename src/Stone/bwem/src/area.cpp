...
vector<int> Area::ComputeDistances(TilePosition start, const vector<TilePosition> & Targets) const
{
	...
	while (!ToVisit.empty())
	{
		...
		const Tile & currentTile = pMap->GetTile(current, no_check);
		bwem_assert(currentTile.Data() == currentDist);
		ToVisit.erase(ToVisit.begin());
		currentTile.SetData(0);										// resets Tile::m_data for future usage
									...
									TilePosition(-1, +1), TilePosition(0, +1), TilePosition(+1, +1)})
		{
			...
			if (pMap->Valid(next))
			{
				const Tile & nextTile = pMap->GetTile(next, no_check); 
				if (!nextTile.Marked())
				{
					if (nextTile.Data())	// next already in ToVisit
					{
						if (newNextDist < nextTile.Data())		// nextNewDist < nextOldDist
						{	// To update next's distance, we need to remove-insert it from ToVisit:
							auto range = ToVisit.equal_range(nextTile.Data());
	...
	// Reset Tile::m_data for future usage
	for (auto e : ToVisit)
		pMap->GetTile(e.second, no_check).SetData(0);	
...
void Area::AddTileInformation(const BWAPI::TilePosition t, const Tile & tile)
{
	...
	if (tile.HigherGround()) ++m_higherGroundTiles;
...
// The function assumes the distance to the nearby ressources has already been computed (in Data()) for each tile around.
// The job is therefore made easier : just need to sum the Data() values.
...
int Area::ComputeBaseLocationScore(TilePosition location) const
{
	...
	{
		const Tile & tile = pMap->GetTile(location + TilePosition(dx, dy), no_check);
		if (!tile.Buildable()) return -1;
		if (tile.Data() == -1) return -1;		// The special value Data() == -1 means there is some ressource at maximum 3 tiles, which Starcraft rules forbid.
...
bool Area::ValidateBaseLocation(TilePosition location, vector<Mineral *> & BlockingMinerals) const
{
	...
	for (int dx = -3 ; dx < dimCC.x + 3 ; ++dx)
	{
		...
		if (pMap->Valid(t))
		{
			const Tile & tile = pMap->GetTile(t, no_check);
...
// To efficiently compute the distances to the ressources, with use Potiential Fields in the Data() value of the Tiles.
	...
	while (!RemainingRessources.empty())
	{
		...
		for (const Ressource * r : RemainingRessources)
			...
			for (int dx = -dimCC.x-max_tiles_between_CommandCenter_and_ressources ; dx < r->Size().x + dimCC.x+max_tiles_between_CommandCenter_and_ressources ; ++dx)
			{
				...
				if (pMap->Valid(t))
				{
					...
					if (tile.AreaId() == Id()) tile.SetData(tile.Data() + score);	// note the additive effect (assume tile.Data() is 0 at the begining)
				}
			}
		...
		for (const Ressource * r : RemainingRessources)
			...
			for (int dx = -3 ; dx < r->Size().x + 3 ; ++dx)
			{
				TilePosition t = r->TopLeft() + TilePosition(dx, dy);
				if (pMap->Valid(t))
					pMap->GetTile(t, no_check).SetData(-1);
			}
		...
		// 5) Clear Tile::m_data (required due to our use of Potential Fields: see comments in 2))
		for (const Ressource * r : RemainingRessources)
			for (int dy = -dimCC.y-max_tiles_between_CommandCenter_and_ressources ; dy < r->Size().y + dimCC.y+max_tiles_between_CommandCenter_and_ressources ; ++dy)
			for (int dx = -dimCC.x-max_tiles_between_CommandCenter_and_ressources ; dx < r->Size().x + dimCC.x+max_tiles_between_CommandCenter_and_ressources ; ++dx)
			{
				TilePosition t = r->TopLeft() + TilePosition(dx, dy);
				if (pMap->Valid(t)) pMap->GetTile(t, no_check).SetData(0);
			}
