...
void Graph::CreateChokePoints()
{
	...
	for (auto & raw : RawFrontierByAreaPair)
	{
		...
		const int cluster_min_dist = (int)sqrt(lake_max_size);
...
vector<int> Graph::ComputeDistances(const ChokePoint * start, const vector<const ChokePoint *> & Targets) const
{
	...
	while (!ToVisit.empty())
	{
		...
		const Tile & currentTile = pMap->GetTile(TilePosition(current->Center()), no_check);
		...
		for (const Area * pArea : {current->GetAreas().first, current->GetAreas().second})
			for (const ChokePoint * next : pArea->ChokePoints())
				if (next != current)
				{
					...
					const Tile & nextTile = pMap->GetTile(TilePosition(next->Center()), no_check); 
					if (!nextTile.Marked())
					{
...
//	bwem_assert(!remainingTargets);
	...
	for (auto e : ToVisit)
		pMap->GetTile(TilePosition(e.second->Center()), no_check).SetData(0);	
...
{
	const Area * pAreaA = GetNearestArea(TilePosition(a));
	const Area * pAreaB = GetNearestArea(TilePosition(b));
	...
	if (pLength) *pLength = minDist_A_B;
